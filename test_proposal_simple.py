...
# 测试 4: JSON 序列化（使用 Pydantic 内置方法）
print("=" * 70)
print("Test 4: JSON Serialization")
print("=" * 70)

# 使用 Pydantic 的 model_dump 方法，mode='json' 会自动处理 datetime
proposal_dict = proposal.model_dump(mode='json', exclude_none=True)
proposal_json = json.dumps(proposal_dict, indent=2, ensure_ascii=False)
print("Proposal JSON (first 800 chars):")
print(proposal_json[:800] + "...")
print()

# 保存完整 JSON 到文件
with open('test_proposal.json', 'w', encoding='utf-8') as f:
    f.write(proposal_json)

print("Full proposal JSON saved to test_proposal.json")
